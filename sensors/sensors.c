 ...
 *
 * Based off of: htc-dream-sapphire sensors.c file
 */
...
static struct driver_t dDriverList[] = {
    {"cma3000-acclerometer",
         "/sys/bus/i2c/drivers/cma3000_accl/4-001c/",
         "/sys/bus/i2c/drivers/cma3000_accl/4-001c/enable",
         "/sys/bus/i2c/drivers/cma3000_accl/4-001c/delay",
         (SENSORS_ACCELERATION) },
    {"sfh7741",
         "/sys/devices/platform/sfh7741.1/",
        ...
        int handle, int enabled)
{
    ...
    LOGD("Changed enabled is: %d", changed_enabled);
    ...
    if (changed_enabled) {
        if (changed_enabled & SENSORS_ACCELERATION) {
	        ...
	        int bytes = sprintf(buffer, "%d\n", flags);
                //LOGD("CMA3000_D0X SET ENABLE: flag = %d", flags);
                if (write(fd, buffer, bytes) < 0) {
                    LOGE("CMA3000_D0X SET ENABLE failed(%s)", strerror(errno));
                    err = -errno;
                }
	...
	err = 0;
        ...
        if (changed_enabled & SENSORS_PROXIMITY) {
	        ...
	        char buffer[20];
	        int bytes = sprintf(buffer, "%d\n", flags);
                //LOGD("SFH7741 set enable: flag = %d", flags);
		if (write(fd, buffer, bytes) < 0) {
                    LOGE("Writing SFH7741 error (%s)", strerror(errno));
                    err = -errno;
                }
                close_dev(dev, ID_SFH, new_enabled, ENABLE_LOCATION);
            } else {
